...
class TabMenu extends Widget
{
    ...
    public function run()
    {
        ...
        Html::addCssClass($this->options, 'nav nav-pills m-1');
    ...
    private function renderBadge($item, $isActive = false)
    {
        ...
        $tag = ArrayHelper::remove($badgeOptions, 'tag', 'span');
        if (!isset($badgeOptions['class'])){
            $type = $isActive ? 'light' : 'primary';
            $badgeOptions['class'] = 'badge badge-'.$type;
        }
